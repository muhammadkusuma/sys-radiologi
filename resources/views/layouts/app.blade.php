    ...
    @auth
        ...
        <aside id="sidebar" class="bg-white border-r border-slate-200 flex flex-col transition-all duration-300 w-64 shrink-0 z-30 fixed inset-y-0 left-0 -translate-x-full md:translate-x-0 md:relative no-print">
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                ...
                @if(Auth::user()->role === 'dokter')
                <a href="{{ route('assessments.history') }}" class="flex items-center px-3 py-2.5 rounded-lg text-sm font-semibold transition-colors {{ Route::is('assessments.history') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                    <svg class="h-5 w-5 flex-shrink-0 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div id="session-expired-modal" class="fixed inset-0 z-50 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-slate-200 relative z-10 p-6 space-y-4">
                ...
                <div>
                    <p class="text-sm text-slate-500">Sesi login Anda tidak valid atau telah kedaluwarsa. Silakan masuk kembali untuk melanjutkan pekerjaan Anda.</p>
                </div>
                <div class="flex justify-end pt-2">
                    <a href="{{ route('login') }}" class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg shadow focus:outline-none cursor-pointer">Login Kembali</a>
                </div>
            </div>
        </div>
    </div>
