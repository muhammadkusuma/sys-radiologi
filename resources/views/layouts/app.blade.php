...
<body class="h-full flex flex-col md:flex-row overflow-x-hidden preload">
    @auth
        <!-- Sidebar -->
        <aside id="sidebar"
            class="bg-white border-r border-slate-200 flex flex-col transition-all duration-300 w-64 shrink-0 z-30 fixed inset-y-0 left-0 -translate-x-full md:translate-x-0 md:relative no-print">
            <!-- Sidebar Header -->
            <div class="h-16 flex items-center justify-between px-4 border-b border-slate-200 flex-shrink-0">
                <a href="{{ route('dashboard') }}" class="flex items-center space-x-3 overflow-hidden">
                    <img id="sidebar-logo" src="/login.png" alt="Logo"
                        class="h-8 w-auto flex-shrink-0 transition-all duration-300">
                </a>
                <button onclick="toggleSidebar()"
                    class="text-slate-400 hover:text-slate-600 focus:outline-none hidden md:block">
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                ...
                <a href="{{ route('dashboard') }}"
                    class="flex items-center px-3 py-2.5 rounded-lg text-sm font-semibold transition-colors {{ Route::is('dashboard') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                ...
                @if (Auth::user()->role === 'dokter')
                    <a href="{{ route('assessments.history') }}"
                        class="flex items-center px-3 py-2.5 rounded-lg text-sm font-semibold transition-colors {{ Route::is('assessments.history') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                        <svg class="h-5 w-5 flex-shrink-0 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span class="sidebar-text truncate">Histori TTD</span>
                    </a>
                @endif


                <a href="{{ route('persetujuan-tindakan.index') }}"
                    class="flex items-center px-3 py-2.5 rounded-lg text-sm font-semibold transition-colors {{ Route::is('persetujuan-tindakan.*') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                    <svg class="h-5 w-5 flex-shrink-0 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                    <span class="sidebar-text truncate">Persetujuan Medis</span>
                </a>

                <!-- Master Pasien -->
                @if (Auth::user()->role === 'perawat' || Auth::user()->role === 'superadmin')
                    <a href="{{ route('patients.index') }}"
                        class="flex items-center px-3 py-2.5 rounded-lg text-sm font-semibold transition-colors {{ Route::is('patients.*') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                        <svg class="h-5 w-5 flex-shrink-0 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @if (Auth::user()->role === 'superadmin')
                    <a href="{{ route('users.index') }}"
                        class="flex items-center px-3 py-2.5 rounded-lg text-sm font-semibold transition-colors {{ Route::is('users.*') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                        <svg class="h-5 w-5 flex-shrink-0 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="p-4 border-t border-slate-200 flex flex-col space-y-3 flex-shrink-0">
                <div class="flex items-center space-x-3 overflow-hidden">
                    <div
                        class="h-9 w-9 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold flex-shrink-0">
                ...
                <form action="{{ route('logout') }}" method="POST" class="w-full">
                    @csrf
                    <button type="submit"
                        class="w-full flex items-center justify-center px-3 py-2 border border-slate-200 text-xs font-semibold rounded-lg text-slate-700 bg-white hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors shadow-sm cursor-pointer">
        ...
        <div id="sidebar-overlay" onclick="toggleMobileSidebar()"
            class="fixed inset-0 bg-slate-900/30 backdrop-blur-sm z-20 hidden"></div>
    @endauth
    ...
    <div class="flex-grow flex flex-col min-h-screen min-w-0 bg-white">
        @auth
            <!-- Mobile Header -->
            <header
                class="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 md:hidden flex-shrink-0 no-print">
        ...
        <footer
            class="bg-white border-t border-slate-200 py-4 text-center text-xs text-slate-500 no-print flex-shrink-0">
    ...
    <div id="confirm-modal" class="fixed inset-0 z-50 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 transition-opacity bg-slate-900/30 backdrop-blur-sm"
                onclick="closeConfirmModal()"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>

            <div
                class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full border border-slate-200 relative z-10 p-6 space-y-4">
                ...
                <div>
                    <p class="text-sm text-slate-500" id="confirm-message">Apakah Anda yakin ingin melakukan tindakan
                        ini?</p>
                </div>
                <div class="flex justify-end space-x-3 pt-2">
                    <button type="button" onclick="closeConfirmModal()"
                        class="px-4 py-2 border border-slate-300 rounded-lg text-sm font-semibold text-slate-700 hover:bg-slate-50 focus:outline-none cursor-pointer">Batal</button>
                    <button type="button" id="confirm-submit-btn"
                        class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-semibold rounded-lg shadow focus:outline-none cursor-pointer">Hapus</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="session-expired-modal" class="fixed inset-0 z-50 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                ...
                <div class="flex items-center space-x-3 text-red-650">
                    ...
                    <h3 class="text-lg font-bold text-slate-900">Sesi Anda Telah Berakhir</h3>
                </div>
                <div>
                    <p class="text-sm text-slate-500">Sesi login Anda tidak valid atau telah kedaluwarsa. Silakan masuk
                        kembali untuk melanjutkan pekerjaan Anda.</p>
                </div>
                <div class="flex justify-end pt-2">
                    <a href="{{ route('login') }}"
                        class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg shadow focus:outline-none cursor-pointer">Login
                        Kembali</a>
                </div>
            </div>
        </div>
    ...
    <div id="analog-clock-picker"
        class="fixed inset-0 z-50 overflow-y-auto hidden flex items-center justify-center p-4">
        <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm" onclick="closeClockPicker()"></div>

        <div
            class="bg-white rounded-2xl shadow-2xl border border-slate-205 w-72 relative z-10 overflow-hidden flex flex-col items-center p-5 space-y-4 select-none">
            <!-- Time Display Header -->
            <div class="flex items-center justify-between w-full">
                <div class="flex items-baseline space-x-1 text-slate-800 font-bold text-4xl">
                    ...
                    <span id="clock-display-minutes"
                        class="cursor-pointer text-slate-400 hover:text-blue-600">00</span>
                </div>
                <div
                    class="text-xs font-bold bg-blue-50 text-blue-600 px-2.5 py-1 rounded border border-blue-100 uppercase tracking-wider">
                    24 Jam</div>
            </div>

            <div class="text-xs font-semibold uppercase tracking-wider text-slate-400 self-start"
                id="clock-mode-title">Pilih Jam</div>

            <!-- Clock Face -->
            <div
                class="relative w-52 h-52 bg-slate-50 rounded-full border border-slate-200 flex items-center justify-center">
                <div class="absolute w-2 h-2 bg-blue-600 rounded-full z-20"></div>

                <div id="clock-hand" class="absolute bg-blue-600 origin-bottom z-10 transition-transform duration-200"
                    style="bottom: 50%; left: calc(50% - 1px); width: 2px; height: 72px; transform: rotate(0deg);">
                    <div
                        class="absolute -top-3 -left-3 w-7 h-7 bg-blue-600 rounded-full flex items-center justify-center text-xs text-white font-bold shadow-sm">
            ...
            <div class="flex justify-end w-full space-x-2 pt-2 border-t border-slate-100">
                <button type="button" onclick="closeClockPicker()"
                    class="px-3.5 py-1.5 border border-slate-300 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-50 cursor-pointer">Batal</button>
                <button type="button" onclick="saveClockPicker()"
                    class="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-xl shadow cursor-pointer">OK</button>
            </div>
        </div>
    </div>
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            document.addEventListener('change', function(e) {
                if (e.target.matches('input[type="number"], input[min], input[max]')) {
                    ...
                    const fieldName = labelCell ? labelCell.textContent.replace(/[*:]/g, '').trim() : input
                        .name;
            ...
            // Session validity checker (runs only for logged in users)
            @auth
